...
;	メニューボタン非表示
	...
	tf.config_x       = [1572, 611,692, 773, 854, 936, 1016, 1098, 1177, 1259, 1340]; // X座標（共通）

	tf.config_y_bgm   = 352; // BGMのY座標
	tf.config_y_se    = 460; // SEのY座標
	tf.config_y_ch    = 593; // テキスト速度のY座標
	tf.config_y_auto  = 700; // オート速度のY座標
...
;	画面右上の「Back」ボタン
	[button fix="true" graphic="&tf.img_path + 'c_btn_back.png'" enterimg="&tf.img_path + 'c_btn_back2.png'" target="*backtitle" x="1760" y="20"]
...
;	未読スキップ-OFF
	[button name="unread_off" fix="true" target="*skip_off" graphic="&tf.btn_path_off" width="250" height="75" x="600" y="837"]

;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="250" height="75" x="873" y="837"]
